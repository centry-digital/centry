...
let card1 = document.getElementById("overview-btn-1");
// Details
...
async function retrieveIncorporationData(emailSave, uuid) {
  ...
  try {
    ...
    if (response.ok && data.status == "Draft") {
      ...
      card1.addEventListener("click", () => tab2.click());
      // co details
    ...
    } else if (response.ok && data.status == "Submitted") {
      form.Btn.innerText = "View";
